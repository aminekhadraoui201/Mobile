...
public class DossierMedicaleActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.profil) {
            ...
            Intent profileIntent = new Intent(DossierMedicaleActivity.this, HopitalActivity.class);
            startActivity(profileIntent);
            return true;
        } else if (item.getItemId() == R.id.deco) {
            // Handle "Deconnexion" (Logout) menu item click
            Intent logoutIntent = new Intent(DossierMedicaleActivity.this, LoginActivity.class);
            startActivity(logoutIntent);
            finish(); // Close the current ProfileActivity
            return true;
        }else if (item.getItemId() == R.id.ajouDon) {
            // Handle "Deconnexion" (Logout) menu item click
            Intent logoutIntent = new Intent(DossierMedicaleActivity.this, AddOrEditRequestActivity.class);
            startActivity(logoutIntent);
            finish(); // Close the current ProfileActivity
            return true;
        }else if (item.getItemId() == R.id.ListeDon) {
            // Handle "Deconnexion" (Logout) menu item click
            Intent logoutIntent = new Intent(DossierMedicaleActivity.this, ViewRequestsActivity.class);
            startActivity(logoutIntent);
            finish(); // Close the current ProfileActivity
            return true;
        }else if (item.getItemId() == R.id.deco) {
